<script lang="ts">
  ...
  import { Volume2, VolumeX, RotateCcw, Share2, Waves, Moon, Sun, Download, Star } from 'lucide-svelte';
...
<div class="min-h-screen flex flex-col justify-between transition-colors duration-400 {dimMode ? 'bg-[#1A1817] text-[#E6DFD5]' : 'bg-[#FAF7F2] text-[#3D3A37]'} selection:bg-rose-100">
  ...
  <header class="w-full max-w-xl mx-auto px-4 py-3 sm:py-5 flex items-center justify-between gap-2">
    <!-- Logo & Title -->
    <div class="flex items-center gap-2 shrink-0">
      <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-2xl bg-[#E07A5F] flex items-center justify-center text-white shadow-xs shrink-0">
        <Waves class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
      </div>
      <div class="text-left">
        <h1 class="text-base sm:text-lg font-semibold tracking-tight {dimMode ? 'text-[#FAF7F2]' : 'text-stone-800'} leading-none">Swell</h1>
        <span class="text-[9px] sm:text-[10px] {dimMode ? 'text-stone-300' : 'text-stone-400'} font-medium tracking-wide uppercase">Contraction Timer</span>
      </div>
    </div>
    ...
    <div class="flex items-center gap-1 sm:gap-1.5 shrink-0">
      ...
      <button
        onclick={toggleDim}
        class="min-w-[38px] sm:min-w-[44px] min-h-[38px] sm:min-h-[44px] p-2 rounded-2xl border transition-colors cursor-pointer flex items-center justify-center shrink-0 {dimMode ? 'bg-stone-800 border-stone-700 text-amber-300 hover:bg-stone-700' : 'bg-white/70 border-stone-200/80 text-stone-600 hover:text-stone-900'}"
      ...
      <button
        onclick={toggleSound}
        class="min-w-[38px] sm:min-w-[44px] min-h-[38px] sm:min-h-[44px] p-2 rounded-2xl border transition-colors cursor-pointer flex items-center justify-center shrink-0 {dimMode ? 'bg-stone-800 border-stone-700 text-stone-300 hover:bg-stone-700' : 'bg-white/70 border-stone-200/80 text-stone-600 hover:text-stone-900'}"
      ...
      {#if contractions.length > 0 || isActive || waterBreak}
        <button
          ...
          class="min-w-[38px] sm:min-w-[44px] min-h-[38px] sm:min-h-[44px] p-2 rounded-2xl border transition-colors cursor-pointer flex items-center justify-center shrink-0 {dimMode ? 'bg-stone-800 border-stone-700 text-stone-400 hover:text-rose-400' : 'bg-white/70 border-stone-200/80 text-stone-400 hover:text-rose-600 hover:bg-rose-50'}"
          title="Reset session"
  ...
  <footer class="w-full text-center py-6 text-xs {dimMode ? 'text-stone-400 border-stone-800 bg-[#1A1817]' : 'text-stone-500 border-[#EBE1D8]/60 bg-[#FAF7F2]'} border-t">
    <div class="flex items-center justify-center gap-1.5 flex-wrap">
      ...
      <span>•</span>
      <a 
        href="https://github.com/kartikp10/swell"
        target="_blank"
        rel="noopener noreferrer"
        class="hover:underline inline-flex items-center gap-1 cursor-pointer"
      >
