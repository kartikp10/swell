<script lang="ts">
  ...
  let { stats, totalCount, onOpenGuide }: Props = $props();
</script>

<div class="w-full max-w-xl mx-auto px-4 mt-6">
  {#if stats.is511RuleMet}
    <div class="mb-4 p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-900 shadow-sm flex items-start gap-3 transition-all animate-gentle-pulse">
      <div class="p-2 rounded-xl bg-rose-100/80 text-rose-600 shrink-0">
        <AlertCircle class="w-5 h-5" />
      </div>
      <div class="flex-1 text-left">
        <h4 class="font-semibold text-sm sm:text-base">5-1-1 Pattern Detected</h4>
        <p class="text-xs sm:text-sm text-rose-700/90 mt-0.5">
  ...
  <div class="grid grid-cols-3 gap-3 bg-white/70 backdrop-blur-md p-4 rounded-3xl border border-[#EBE1D8] shadow-xs">
    <!-- Frequency / Interval -->
    <div class="flex flex-col items-center justify-center p-3 rounded-2xl bg-[#FAF7F2]/80 text-center">
      ...
      <span class="text-xl sm:text-2xl font-semibold text-stone-800">
  ...
  <div class="mt-2 text-right">
    <button
      onclick={onOpenGuide}
      class="text-xs font-medium text-stone-500 hover:text-[#E07A5F] underline underline-offset-4 transition-colors"
    >
